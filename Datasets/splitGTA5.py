...
def split80_20(root):
    ...
    train_indexes, val_indexes = train_test_split(
        list(range(len(samples))), test_size=0.20, random_state=42
    )
...
def move_dir(samples, dir, mode):
    ...
    for sample in tqdm(samples, desc=f"{dir=} {mode=}"):
        shutil.move(sample[0], images_dir)
        shutil.move(sample[1], labels_dir)


if __name__ == "__main__":
    train_subset, val_subset = split80_20(GTA5_BASE_PATH)
    move_dir(train_subset, GTA5_BASE_PATH, "train")
    move_dir(val_subset, GTA5_BASE_PATH, "val")
